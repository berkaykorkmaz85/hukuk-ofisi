...
function renderRaporlarPage() {
  ...
  const mvOpts  = muvekkiller.map(m=>`<option value="${escHtml(m.ad)}">${escHtml(m.ad)}</option>`).join('');
...
function exportData() {
  ...
  const d = new Date();
  a.href = url;
  a.download = `hukuk-yedek-${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}.json`;
  a.click();
  URL.revokeObjectURL(url);
...
async function exportDataEncrypted() {
  ...
  try {
    ...
    const d = new Date();
    a.href = url;
    a.download = `hukuk-yedek-sifreli-${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}.json`;
    a.click();
    URL.revokeObjectURL(url);
...
function onIcraIlChange() {
  ...
  if (adliyeler && adliyeler.length > 1) {
    ...
    adliyeSel.innerHTML = il ? `<option value="${escHtml(il)}">${escHtml(il)}</option>` : '';
...
function onAdliyeIlChange() {
  ...
  if (adliyeler && adliyeler.length > 1) {
    ...
    sel.innerHTML = `<option value="${escHtml(il)}">${escHtml(il)}</option>`;
    wrap.style.display = 'none';
  }
...
function updateKisilerDatalist() {
  ...
  if (!dl) return;
  const kisiler = DB.get('kisiler');
  dl.innerHTML = kisiler.map(k => `<option value="${escHtml(k.ad)}">${escHtml(k.ad)} (${escHtml(k.rol)})</option>`).join('');
}
