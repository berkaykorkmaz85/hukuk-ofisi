...
// ══ GÖREVLER SATIR LİSTESİ (dava + icra detay sayfaları ortak) ══
function _gorevDurum(t) {
  if (t.done) return 'tamam';
  if (t.kanbanDurum === 'devam') return 'devam';
  return 'bekleyen';
}

function _gorevRerender(ctxType, id) {
  if (ctxType === 'dava') renderDavaTab(id, 'gorev');
  else renderIcraTab(id, 'gorev');
}

function _gorevDurumDongu(taskId, ctxType, id) {
  var cycle = ['bekleyen', 'devam', 'tamam'];
  var arr = DB.get('tasks').map(function(t) {
    if (t.id !== taskId) return t;
    var mevcut = _gorevDurum(t);
    var yeni = cycle[(cycle.indexOf(mevcut) + 1) % cycle.length];
    var upd = Object.assign({}, t, { kanbanDurum: yeni });
    upd.done = yeni === 'tamam';
    return upd;
  });
  DB.set('tasks', arr);
  _gorevRerender(ctxType, id);
}

function _gorevDurumBadge(durum) {
  var map = {
    bekleyen: { icon: '📍', label: 'Bekleyen', renk: 'var(--gold)' },
    devam: { icon: '🔄', label: 'Devam Ediyor', renk: '#7ab5d4' },
    tamam: { icon: '✅', label: 'Tamamlandı', renk: 'var(--green)' }
  };
  return map[durum] || map.bekleyen;
}
