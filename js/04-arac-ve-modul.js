...
function _uetsYeniKayitAc() {
  ...
  sel.innerHTML = '<option value="">— Dosya seçin (opsiyonel) —</option>'
    + '<optgroup label="📁 Davalar">' + davalar.map(d => `<option value="${escHtml(d.no)}">${escHtml(d.no)} — ${escHtml(d.muvekkil||'')}</option>`).join('') + '</optgroup>'
    + '<optgroup label="⚡ İcralar">' + icralar.map(i => `<option value="${escHtml(i.no)}">${escHtml(i.no)} — ${escHtml(i.borclu||'')}</option>`).join('') + '</optgroup>';
...
function _uetsDuzenle(id) {
  ...
  const icralar = DB.get('icralar') || [];
  sel.innerHTML = '<option value="">— Dosya seçin (opsiyonel) —</option>'
    + '<optgroup label="📁 Davalar">' + davalar.map(d => `<option value="${escHtml(d.no)}"${d.no===k.dosyaNo?' selected':''}>${escHtml(d.no)} — ${escHtml(d.muvekkil||'')}</option>`).join('') + '</optgroup>'
    + '<optgroup label="⚡ İcralar">' + icralar.map(i => `<option value="${escHtml(i.no)}"${i.no===k.dosyaNo?' selected':''}>${escHtml(i.no)} — ${escHtml(i.borclu||'')}</option>`).join('') + '</optgroup>';
  _uetsSureSec(sure);
  openModal('modal-uets');
}
...
function tsIlceChange() {
  ...
  if (ilce) mahSel.innerHTML += `<option value="${escHtml(ilce)} Merkez Mahallesi">${escHtml(ilce)} Merkez Mahallesi</option>`;
}
...
function populateMuvekkilSelects() {
  ...
  ids.forEach(id => {
    ...
    if (!sel) return;
    const cur = sel.value;
    sel.innerHTML = '<option value="">Seçin...</option>' + mv.map(m=>`<option value="${escHtml(m.ad)}">${escHtml(m.ad)}</option>`).join('');
    sel.value = cur;
  });
}
...
function tabRender() {
  ...
  bar.innerHTML = _tabs.map(function(t) {
    ...
    return '<div class="tab-item ' + typeClass + (isActive ? ' active' : '') + '" '
      ...
      + 'title="' + escHtml(fullLabel) + '" data-tab-id="' + t.id + '">'
...
function finansModalMvDegisti() {
  ...
  karsiDosya.innerHTML = '<option value="">— Dosya seçin —</option>'
    + (davalar.length ? '<optgroup label="📁 Dava Dosyaları">'
      + davalar.map(function(d){ return '<option value="'+escHtml(d.no)+'">'+escHtml(d.no)+' — '+escHtml(d.muvekkil)+(d.karsi?' vs '+escHtml(d.karsi):'')+'</option>'; }).join('')
      + '</optgroup>' : '')
    + (icralar.length ? '<optgroup label="⚡ İcra Dosyaları">'
      + icralar.map(function(i){ return '<option value="'+escHtml(i.bki||i.no)+'">'+escHtml(i.bki||i.no)+' — '+escHtml(i.borclu)+'</option>'; }).join('')
      + '</optgroup>' : '');
}
...
function finasModalMvCari() {
  ...
  if (sel) {
    const icralar = DB.get('icralar');
    sel.innerHTML = [...davalar.map(d=>`<option value="${escHtml(d.ad||d.no)}">`), ...icralar.filter(i=>i.muvekkil===mv).map(i=>`<option value="${escHtml(i.no)}">`)].join('');
  }
...
function chRenderEkler(ekler) {
  ...
  ekler.forEach(function(ek) {
    ...
    if (isResim) {
      ...
      html += '<a href="' + safeUrl + '" target="_blank" class="ch-thumb-card" title="' + escHtml(ek.ad) + '">'
